...
% 3 - Compute the unregularized cost
K = num_labels;
for i = 1:m
    for k = 1:K
        J = J + (-y_matrix(i, k)*log(a3(i, k)) -...
            (1 - y_matrix(i, k))*log(1 - a3(i, k)));
    end
end
J = J / m;

% 4 - Compute the regularized component of the cost
t = 0;
for j = 1:size(Theta1, 1)
    for k = 2:size(Theta1, 2)
      t = t + Theta1(j, k) * Theta1(j, k);
    end
end
for j = 1:size(Theta2, 1)
    for k = 2:size(Theta2, 2)
      t = t + Theta2(j, k) * Theta2(j, k);
    end
end
J = J + t * lambda / (2 * m);
